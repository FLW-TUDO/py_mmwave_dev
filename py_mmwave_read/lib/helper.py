...
#from py_mmw_main import configParameters
...
def log2lin(NumVirtAnt):
    log_to_lin = (1 / 512) * (2 ** np.ceil(np.log2(NumVirtAnt)) / NumVirtAnt)
    return log_to_lin

def dspFftScaleCompAll_log(cfg_numRangeBins):
    return 20 * np.log10(32 / cfg_numRangeBins)


def rp_to_db(value, cfg_numRangeBins):
    log_to_lin = log2lin(12)
    toDB = 20 * np.log10(2)
    dspFftScaleCompAll = dspFftScaleCompAll_log(cfg_numRangeBins)
    #print(f"log_to_lin: {log_to_lin}, toDB: {toDB}, dspFftScaleCompAll: {dspFftScaleCompAll}")
    result = np.round(((value*log_to_lin*toDB)+(dspFftScaleCompAll)),3)
    return result
# ------------------------------------------------
