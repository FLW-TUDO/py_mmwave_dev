...
class RadarApp:
    def __init__(self, configFileName, controlPort_, dataPort_, filename):
        try:
            self.config_parameters = parseConfigFile(configFileName)
            self.data_parser = DataParser(self.config_parameters, filename)
            self.CLIport, self.Dataport = serialConfig(configFileName, controlPort_, dataPort_)
        ...
        # self.data_parser = DataParser(self.config_parameters, filename)
        # self.CLIport, self.Dataport = serialConfig(configFileName, controlPort_, dataPort_)
        # print(f"RadarApp is initiated! {self.config_parameters}, {self.CLIport}")
        # #send_reset_command(self.CLIport)
...
def update_non_plot(self):
    ...
    dataOk, frameNumber, detObj = self.data_parser.readAndParseData68xx(self.Dataport, self.configParameters)
    print("update dataOk: ", dataOk, "detObj: ", detObj)
                
    if dataOk and \
        self.data_parser.configParameters["detectedObjects"] and \
        len(detObj["x"]) > 0:
...
# Signal handler to ensure ports are closed on exit
def signal_handler(sig, frame):
    global running
    running = False
    print("Ctrl+C pressed. Exiting...")
    if app_instance.CLIport.is_open:
        app_instance.CLIport.write(('sensorStop\n').encode())
        app_instance.CLIport.close()
        print("CLIport closed")
    if app_instance.Dataport.is_open:
        app_instance.Dataport.close()
        print("Dataport closed")
    if visualizer:
        QtWidgets.QApplication.quit()
    sys.exit(0)
...
def main():
    ...
    # Main loop 
    try:
        app_instance = RadarApp(configFileName, controlPort_, dataPort_, filename)
        ...
        if nrst:
            send_reset_command(app_instance.CLIport)
        else:
        ...
        while running:
            ...
            #dataOk = update_non_plot(app_instance)
            dataOk = app_instance.update_non_plot()
            #print("detObj: ", detObj)
