...
_meta_ = {
    ...
    'app': {
        ...
        'detectedObjects':     ('plot_detected_objects', 'simple_cfar_clustering',),  # 'simple_cfar_clustering',),
...
def _init_(prt, dev, cfg, dat):
    ...
    taux = threading.Thread(target=_data_, args=(aux,)) #execute _data_ in while loop
    taux.start()
...
#proceed with the apps for plotting guiMonitor
...
def _pipe_(dat):
    for tag in apps:
        ...
        try:
            ...
            #print("pipe: ", apps[tag], tag, str.encode(dat + '\n'))
        ...
        try:
            ...
            if data[:len(_meta_['seq'])] == _meta_['seq']:  # check for magic sequence
                if len(output) > 0:
                    ...
                    _pipe_(plain) #passing the processed output data to _pipe_
                    #print("_data_pipe: ", plain)
                    if verbose:
                ...
                while flen < len(input['buffer']):  # keep things finite
                    ...
                    if len(output) == 0:    # filter out empty and duplicate frames from log
                        if dataFramePrev.setdefault('header', {}).setdefault('objects', 0) > 0:
                            #print("_data_ true: ", dataFramePrev)
                            log.message(dataFramePrev)
                            #print("test: ", dataFramePrev)
                    else:
                        dataFramePrev = output
                        #print("_data_ else: ", output)
...
def aux_buffer(input, output, head=40, indices={
    ...
    def aux_object_rd(dat, oth, n=16):  # detected points/objects
        #x = struct.unpack('f',dat[ 0: 4])[0]
        #y = struct.unpack('f',dat[ 4: 8])[0]
        #z = struct.unpack('f',dat[ 8:12])[0]
        #p = struct.unpack('f',dat[12: n])[0]
    ...
    def aux_side_info(dat, n=4):  # side information
        snr = intify(dat[0:2])
        noise = intify(dat[2:4])
        return n, snr, noise
    ...
    #elif address == 5 and len(buffer) >= 2 and values > 0:
    ...
    #elif address == 2 and len(buffer) >= 2 and values > 0:
        #print("address: ", address)
        n, v = aux_profile(buffer)
        progress(n, indices[address], q_to_db(v))
        #print("log msg range: ", n, indices[address], q_to_db(v))
    
    # 1) point cloud
    # Ori --------------------
    while address == 1 and len(buffer) >= 16 * output['header']['objects'] and values > 0:
    #elif address == 1 and len(buffer) >= 16 * output['header']['objects'] and values > 0:
        #print("address: ", address)
        numPoints = output['header']['objects']
        for i in range(numPoints):
            n, p, x, y, z = aux_object(buffer, other)
            progress(n, indices[address], ('{},{}'.format(i, i), {'v': p, 'x': x, 'y': y, 'z': z}))
            #print("point cloud: ", n, indices[address], ('{},{}'.format(i, i), {'v': p, 'x': x, 'y': y, 'z': z}))
    #-----------------------------
    # elif address == 1 and len(buffer) >= 16 * output['header']['objects'] and values > 0:
    #     numPoints = output['header']['objects']
    #     points_processed = 0
    #     #print(points_processed, numPoints)
    #     while len(buffer) >= 16 and points_processed < numPoints and values > 0:
    #         n, p, x, y, z = aux_object(buffer, other)
    #         progress(n, indices[address], ('{},{}'.format(points_processed, points_processed), {'v': p, 'x': x, 'y': y, 'z': z}))
    #         #print('x: ', x, 'y: ', y, 'z: ', z, 'v: ', p)
    #         points_processed += 1
    #         numPoints -= 1
    #         if numPoints == 0:
    #             address = 0
    ...
    # #     try:
    # #         if block in output:
    # #             if isinstance(output[block], list):
    # #                 output[block].append(value)
    # #             else:
    # #                 output[block].update(value)
    # #         else:
    # #             output[block] = [value] if isinstance(value, list) else value
    # #     except:
    # #         output[block] = [value] if isinstance(value, list) else value
    # #     if verbose:
    # #         print(f"Processed {block}: {value}")
    # def process_point_cloud():
    #     nonlocal address, values
    #     numPoints = output['header']['objects']
    #     points_processed = 0
    #     print("process point cloud")
    #     #while address == 1 and len(buffer) >= 16 * numPoints and values > 0 and points_processed < 10:
    #         #numPoints = output['header']['objects']
    #     for i in range(numPoints):
    #         n, p, x, y, z = aux_object(buffer, other)
    #         progress(n, indices[address], ('{},{}'.format(i, i), {'v': p, 'x': x, 'y': y, 'z': z}))
    #         points_processed += 1
    #     numPoints -= points_processed

   
    # if len(buffer) >= 8 and blocks > 0 and address == 0:
    #     print("buffer >= 8", address)
    #     n, address, values = aux_struct(buffer)
    #     buffer = buffer[n:]
    #     blocks -= 1
    #     if address in (1, 7):
    ...
    #     elif address in (6, ):
    #         output[indices[address]] = None
    # elif len(buffer) >= head and blocks == -1 and address == 0 and values == 0:
    #     #print("init head")
    #     n, v, l, d, f, t, o, s, u = aux_head(buffer)
    #     buffer = buffer[n:]
    #     blocks = s
    ...
    # elif address == 7 and len(buffer) >= 4 and values > 0:
    #     print(address)
    #     buffer = buffer[4:]
    #     values -= 1
    #     if values == 0: address = 0
    ...
    # elif address == 6 and len(buffer) >= 24 and values > 0:
    #     print(address)
    #     n, ifpt, tot, ifpm, icpm, afpl, ifpl = aux_info(buffer)
    #     progress(n, indices[address], {
    #         'interframe_processing': ifpt,
    ...
    # input['values'] = values
    # input['other'] = other
    # #print("main loop: ", address, "val: ", buffer)

    # # if len(buffer) < 2 and address == 0:
    # #     break
    
    #----------------------------------------
        
    # original version ----------
    #print("address aux_buffer: ", address)
    # 7) point cloud side info
    # while address == 7 and  len(buffer) >= 4 and values > 0:
    #     buffer = buffer[4:]  # TODO
    #     print("address 7: ". buffer)
    ...
    #     if values == 0: address = 0
    # Process point cloud side info
    # while address == 7 and len(buffer) >= 4 * output['header']['objects'] and values > 0:
    #     print("address 7")
    #     numPoints = output['header']['objects']
    #     for i in range(numPoints):
    #         n, snr, noise = aux_side_info(buffer)
